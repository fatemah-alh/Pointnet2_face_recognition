...
parser.add_argument('--log_dir', default='/data/falhamdoosh/log', help='Log dir [default: log]')
parser.add_argument('--num_point', type=int, default=6704, help='Point Number [default: 1024]')
...
NUM_CLASSES = 7 # 35for essprssione , 105 for id , 40 for modelnet,5 for unicod ,33for espression test
...
if FLAGS.normal:
    ...
    NUM_CHANNEL=3
    normal=False
print("number of channel," , NUM_CHANNEL)
   # assert(NUM_POINT<=10000)
DATA_PATH =  '/data/falhamdoosh/face_with_normal'
TRAIN_DATASET = face_dataset.FaceRecognitionDataset(os.path.join(DATA_PATH,"trainfiles.txt"), npoints=NUM_POINT,NUM_CHANNEL=NUM_CHANNEL, batch_size=BATCH_SIZE,shuffle=True)
TEST_DATASET = face_dataset.FaceRecognitionDataset(os.path.join(DATA_PATH,"testfiles.txt"), npoints=NUM_POINT,NUM_CHANNEL=NUM_CHANNEL, batch_size=BATCH_SIZE,shuffle=False)
...
def train():
    with tf.Graph().as_default():
        with tf.device('/cpu:0'):
            pointclouds_pl, labels_pl = MODEL.placeholder_inputs(BATCH_SIZE, NUM_POINT,normal)
            ...
            MODEL.get_model(pointclouds_pl, is_training_pl,NUM_CLASSES,normal=normal, bn_decay=bn_decay)
            ...
            for i in range(NUM_GPUS):
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    with tf.device('/gpu:%d'%(i)), tf.name_scope('gpu_%d'%(i)) as scope:
                        ...
                        pred, end_points = MODEL.get_model(pc_batch, is_training=is_training_pl, NUM_CLASSES=NUM_CLASSES,normal=normal , bn_decay=bn_decay)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    cur_batch_data = np.zeros((BATCH_SIZE,NUM_POINT,NUM_CHANNEL))
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    print  (np.array(total_correct_class,dtype=np.float)/np.array(total_seen_class,dtype=np.float))
